...
| Values beyond the 1.5 IQR fences | 0–75 per column | kept, not winsorized |
...
The six categorical columns arrived as plain text and are converted to factors. Reference levels are set to the healthy / unexposed group (`Female`, `Asymptomatic`, `No`, `Never`) so the coefficients in Step 3 read as "the effect of being exposed". `smoker_status` (Never < Former < Current), `bp_category`, and `glycemic_status` are stored as **ordered** factors because their levels have a genuine ordering.
...
| `percent_predicted_max_hr` | 100 × peak / (220 − age) | Age fixes the attainable peak (r = −0.73); this rescales it to an age-free measure of effort tolerance |
| `bmi_category` | Underweight / Normal / Overweight / Obese | WHO cut-offs |
| `bp_category` | Normal / Elevated / Hypertension stage 1 / stage 2 | ACC–AHA stages, taking the higher of what systolic and diastolic imply |
| `glycemic_status` | Normal / Prediabetic / Diabetic | HbA1c thresholds of 5.7 % and 6.5 % |
